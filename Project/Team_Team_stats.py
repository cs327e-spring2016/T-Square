...
NEW_TEAM_ABBREV_LIST = ['CHO','BRK','NOP']

NEW_TEAM_NAME = {'Charlotte Hornets':'CHO','Brooklyn Nets':'BRK','New Orleans Pelicans':'NOP'}

NEW_TEAM_NAME_LIST = ['Charlotte Hornets','Brooklyn Nets','New Orleans Pelicans']

TEAM_TABLE_COLUMN_NAME = ['name','abbrev']

def get_team_info(TEAM_ABBREV,TEAM_NAME,cur):
	for i in range (len(TEAM_ABBREV)):
		team_info = []
		team_info.append(TEAM_NAME[i])
		team_info.append(TEAM_ABBREV[i])
		output1 = create_insert_string("team",team_info,TEAM_TABLE_COLUMN_NAME)
		cur.execute(output1)
		cur.connection.commit()

def team_table (NEW_TEAM_ABBREV_LIST,NEW_TEAM_NAME_LIST,cur):
	for i in range (len(NEW_TEAM_ABBREV_LIST)):
		team_info = []
		team_info.append(NEW_TEAM_NAME_LIST[i])
		team_info.append(NEW_TEAM_ABBREV_LIST[i])
		output1 = create_insert_string("team",team_info,TEAM_TABLE_COLUMN_NAME)
		cur.execute(output1)
		cur.connection.commit()
		...
		for team in TEAM_ABBREV:
			...
			if(srapping_game_data(leb) == False):
				...
				for i in range(len(rows)):
					if (rows[i][1] != "Rk" and len(rows[i])>10):
						all_team_stats_info.append(get_team_stats_info(rows[i],team))
		#insert date into team_stats table
		team_stats_table(all_team_stats_info,cur)

		#insert date into team table
		get_team_info(TEAM_ABBREV,TEAM_NAME,cur)
		team_table (NEW_TEAM_ABBREV_LIST,NEW_TEAM_NAME_LIST,cur)
	cur.close()
	conn.close() 
main()